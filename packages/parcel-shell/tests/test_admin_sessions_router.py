...
async def test_list_sessions(authed_client: AsyncClient, user_factory) -> None:
    ...
    vic_client = httpx.AsyncClient(transport=authed_client._transport, base_url="http://t")
    await vic_client.post("/auth/login", json={"email": "vic@x.com", "password": "password-1234"})
...
async def test_revoke_all_sessions(authed_client: AsyncClient, user_factory) -> None:
    import httpx

    vic = await user_factory(email="v2@x.com", password="password-1234")
    vic_client = httpx.AsyncClient(transport=authed_client._transport, base_url="http://t")
    await vic_client.post("/auth/login", json={"email": "v2@x.com", "password": "password-1234"})
