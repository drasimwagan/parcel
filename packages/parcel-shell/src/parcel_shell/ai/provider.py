...
def _validate_path(path: str) -> None:
    ...
    if any(
        ...
        for ext in (".sh", ".exe", ".so", ".dll", ".dylib")
    ):
...
class AnthropicAPIProvider:
    ...
    async def generate(
        ...
        *,
        ...
        for _ in range(_MAX_TOOL_ITERATIONS):
            ...
            for tu in tool_uses:
                ...
                if len(data) > MAX_FILE_BYTES:
                    raise ProviderError(
                        f"write_file content too large: {len(data)} bytes"
                    )
                total_bytes += len(data)
                if total_bytes > MAX_TOTAL_BYTES:
                    raise ProviderError(
                        f"total generated bytes too large: {total_bytes}"
                    )
                files[path] = data
                tool_results.append(
                    {
        ...
        if not submitted:
            raise ProviderError(
                "exceeded tool-use iteration cap without submit_module"
            )
        if not files:
    ...
    async def generate(
        ...
        *,
        ...
        try:
            ...
            raise ProviderError(
                f"claude CLI timeout after {self._timeout_s}s"
            ) from exc
        ...
        if not files:
            raise ProviderError(
                "claude CLI produced no files in the working directory"
            )
