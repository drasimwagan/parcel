...
**Phase 10c — Workflows rich actions + UI done.** Four new action types ship: `SendEmail(to, subject, body)` (stdlib `smtplib` via `asyncio.to_thread`; required capability `network`; SMTP host configured via `PARCEL_SMTP_HOST` etc; raises when not configured), `CallWebhook(url, method, headers, body)` (httpx 0.27+; non-2xx raises; required capability `network`), `RunModuleFunction(module, function)` (looks up `Module.workflow_functions[function]` and awaits with `WorkflowContext`; return value truncated to 1 KiB and stored in payload), and `GenerateReport(module, slug, params)` (resolves the Phase-9 report registry, validates Pydantic params, renders the report's HTML body and stashes in payload, truncated to 32 KiB). Each action class carries a `_required_capability: ClassVar[str | None]` ClassVar; `mount_module` warns `module.workflow.missing_capability` when a workflow's action declares a capability the module hasn't put in `Module.capabilities`. New SDK field `Module.workflow_functions: dict[str, Callable[[WorkflowContext], Awaitable[Any]]] = {}` registers the callables `RunModuleFunction` invokes. New manual-retry route `POST /workflows/<m>/<s>/retry/<audit_id>` re-runs failed audits (inline mode uses `attempt = audit.attempt + 1`; queued mode enqueues `run_event_dispatch` with a fresh `job_try=1` — chronological audit ordering still tells the retry story). Audit detail page accepts `?status=ok|error&event=<substring>` query params with server-side filtering and pre-filled form. Detail template gains an `Actions` column with per-row Retry buttons on errored audits. SDK bumped to `0.9.0`; Contacts bumped to `0.6.0` and ships an `audit_log_via_function` reference workflow + `audit_log` registered in `workflow_functions`. New shell runtime dep: `httpx>=0.27,<1.0`. Test count: ~430 → ~450.

Next: **Phase 11 — Sandbox preview enrichment** (sample-record seeding via Claude-generated `seed.py`, Playwright screenshots of every declared route at three viewport sizes, surfaced in the sandbox detail page; rides ARQ from Phase 10b). Start a new session; prompt: "Begin Phase 11 per `CLAUDE.md` roadmap." This is the last item on the workflow-and-module-authoring axis before "Future" concerns (multi-tenancy, OIDC/SAML, module registry, in-browser developer module).
...
| 11 | ⏭ next | Sandbox preview enrichment — sample-record seeding, Playwright screenshots, builds on ARQ |
