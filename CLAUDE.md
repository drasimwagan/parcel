...
| Workflow rich actions | Four 10c action types: `SendEmail` (stdlib smtplib via to_thread; cap `network`), `CallWebhook` (httpx; cap `network`), `RunModuleFunction` (looks up `Module.workflow_functions[name]`), `GenerateReport` (renders Phase-9 report HTML into payload). Each class has `_required_capability: ClassVar[str \| None]`. Late imports inside `runner.execute_action` keep heavy deps off the SDK-test code path. |
| Module workflow_functions | New `Module.workflow_functions: dict[str, Callable[[WorkflowContext], Awaitable[Any]]] = {}`. `RunModuleFunction(module, function)` looks up by name and awaits with the context. Return value coerced to `str(...)[:1024]` for safe payload storage. |
| Workflow capability mount-warn | `mount_module` walks each workflow's actions and emits `module.workflow.missing_capability` at WARN when an action's `_required_capability` isn't in `Module.capabilities`. Same model as the existing permission warnings — log + continue. AI-generator-time enforcement lands in Phase 11. |
| Workflow manual-retry route | `POST /workflows/<module>/<slug>/retry/<audit_id>` permission-gated. 404 on missing audit, cross-workflow audit, or `status="ok"` audit. Inline mode runs `run_workflow` directly with `attempt = audit.attempt + 1`. Queued mode enqueues `run_event_dispatch` (fresh `job_try=1`). Chronological audit ordering still tells the retry story; documented imprecision. |
| Workflow audit filter | Detail page accepts `?status=ok\|error&event=<substring>` query params. Server-side WHERE clauses on the existing audits select. Filter form rendered above the table with pre-fill from query state and a "clear" link. |
| Workflow Retry button | Detail template's audit table gains an `Actions` column. Per-row inline form posting to the retry route, rendered only when `status="error"`. |
| Workflow SMTP settings | `Settings.smtp_host` (None → SendEmail raises), `_port` (587), `_username`, `_password`, `_from_address`. All optional. |
| Sandbox preview runner | `render_sandbox_previews(sandbox_id)` ARQ job, single Chromium per render, three viewport contexts. Inline mode uses `asyncio.create_task` mirroring AI chat's `run_turn` pattern. Inline tasks tracked in `app.state.preview_tasks`, cancelled on lifespan exit. Orphan sweep on boot flips any `preview_status='rendering'` to `failed(process_restart)`. |
| Sandbox preview routes (auto-walk) | `routes.resolve` auto-walks `module.router.routes` for GET/HTML routes, skips JSON-only `response_class`, fabricates path-param values from `module.metadata.tables` first-row PKs. `Module.preview_routes` (new SDK field, type `tuple[PreviewRoute, ...] = ()`) overrides the auto-walk when supplied. |
| Sandbox preview seed | `<module_root>/src/parcel_mod_<name>/seed.py` with `async def seed(session)`, gated like the rest of the module. Optional — modules without seed.py render against an empty schema. No synthetic fallback in 11. |
| Sandbox preview identity | Fixed-UUID `sandbox-preview@parcel.local` user (`00000000-0000-0000-0000-000000000011`) + `sandbox-preview` builtin role (migration 0009); permissions synced at render-time via `identity.sync_preview_role` (NOT seeded by the migration — runtime sync keeps the role aligned with installed modules). Hidden from `/users` and `/roles` listings; mutations and deletes return 403. |
| Sandbox preview storage | `var/sandbox/<uuid>/previews/<sha1prefix>_<viewport>.png`. Filename whitelisted against the `previews` JSONB roster on every image-serving request. `dismiss_sandbox` cleans everything via the existing `module_root` rmtree. |
...
### Phase 10c — Workflows rich actions + UI ✅ shipped

Shipped on the `phase-10c-rich-actions` branch. See the seven new "Workflow *" rows added in this phase under "Locked-in decisions" for the concrete contracts. Four action types (`SendEmail`, `CallWebhook`, `RunModuleFunction`, `GenerateReport`) each carry a `_required_capability` ClassVar; mount-time warns when the module hasn't declared the capability. New `Module.workflow_functions` dict registers async callables that `RunModuleFunction` invokes. Manual-retry route + per-row Retry button on errored audits. Audit filter UI (`?status=&event=`). httpx promoted to runtime dep; SMTP settings (`PARCEL_SMTP_*`) added. Contacts ships `audit_log_via_function` reference workflow. AI-generator-time capability enforcement deferred to Phase 11's static-analysis gate work.

### Phase 11 — Sandbox preview enrichment ✅ shipped

Shipped on the `phase-11-sandbox-previews` branch. See the eight new "Sandbox preview *" rows under "Locked-in decisions" for the concrete contracts. Architecture landed as a `parcel_shell.previews` package (`runner` / `routes` / `seed_runner` / `storage` / `identity` / `queue` / `worker`) driven by the ARQ infrastructure from Phase 10b. Inline mode short-circuits to `asyncio.create_task` and is what powers the test suite + `parcel dev`. Auto-walk of `module.router.routes` resolves preview targets without any module-side declaration; `Module.preview_routes` is the override hatch when a module needs custom path params. Modules can ship an optional `async def seed(session)` to populate sample records before the screenshot pass. A fixed-UUID `sandbox-preview` system identity (migration 0009) gives Playwright a cookie session; the user + role are hidden from `/users` / `/roles`. Three new HTTP routes (fragment, render, image) plus an Alpine-driven viewport tab strip in the sandbox detail page. New CLI `parcel sandbox previews <uuid>`. Synthetic seed fallback (originally on the brainstorm list) was deferred — empty-schema previews are the fallback in Phase 11, and it's been adequate for the Contacts demo.

**Known Phase 11 follow-ups (non-blocking, land opportunistically):**

- **AI-generator prompt update for `seed.py`.** The Claude generator's system prompt at `packages/parcel-shell/src/parcel_shell/ai/prompts/generate_module.md` does not yet instruct the model to emit a `seed.py`. Add a short section so AI-generated modules ship sample data alongside their schema.
...
- Don't add multi-tenancy, OIDC/SAML, or a module registry — all three are in the Future row and stay there until the platform has real users driving the priority. Everything shipped through Phase 11 assumes single-tenant, cookie-session auth, and local module discovery.
