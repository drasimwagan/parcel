...
**Phase 9 — Reports + PDF generation done.** Modules can now declare `Report(...)` tuples on their manifest; the shell auto-mounts three URLs per report — `/reports/<module>/<slug>` (parameter form), `/reports/<module>/<slug>/render?<params>` (HTML preview wrapped in admin chrome), and `/reports/<module>/<slug>/pdf?<params>` (streamed PDF download). Pydantic param models drive an auto-rendered Tailwind form covering `str`/`int`/`float`/`bool`/`date`/`datetime`/`Literal`/`Enum` plus a `widget=textarea` escape hatch and a `Report.form_template` override for anything more exotic. PDFs render via WeasyPrint 62.x (pure Python, CSS print model — no browser subprocess); the import is lazy so the shell still boots on Windows dev machines without GTK runtime, and PDF tests skip there. Module reports `{% extends "reports/_report_base.html" %}` for A4 portrait, 20mm margins, page-counter footer; `{% block page_css %}` overrides for landscape/Letter. Per-report permission gating (`Report.permission` is a permission the module already owns); 404 (not 403) on missing permission, no new shell permissions, no new shell migrations. Boot-time WARN (`module.report.unknown_permission`) when a declared report points at a permission the module doesn't own. Validation errors re-render the form inline; data-fn / template-render failures surface a friendly red error block; PDF engine failures redirect with a flash. SDK bumped to `0.5.0` (adds `Report`, `ReportContext`); Contacts bumped to `0.3.0` and ships a reference `contacts.directory` report with `company` / `created_after` / `created_before` filters. Sidebar auto-injects per-report links (one entry per visible report). Test count climbs from 296 → ~336 (PDF render itself is the only test that skips locally on Windows; runs in Docker).

Next: **Phase 10 — Workflows** (state-machine declarations, triggers, actions, audit log; ARQ becomes first-class infrastructure with a worker container). Start a new session; prompt: "Begin Phase 10 per `CLAUDE.md` roadmap." Do not begin Phase 10 inside the Phase 9 commit cluster. The full upcoming roadmap (9 Reports/PDF ✅ → 10 Workflows → 11 Sandbox preview enrichment) is described below under "Upcoming phases".
...
| Report PDF engine | **WeasyPrint 62.x** (pure Python, CSS print model, no JS, no browser subprocess). `parcel_shell.reports.pdf.html_to_pdf` lazy-imports `weasyprint` so the shell still boots on Windows dev machines without GTK runtime; `base_url` is the shell's package root as a `file://` URI so `/static/...` references resolve. Native libs (`libpango-1.0-0`, `libcairo2`, `libgdk-pixbuf-2.0-0`, `libffi-dev`, `shared-mime-info`, `fonts-dejavu-core`) are installed in the Docker image. PDF tests skip locally when WeasyPrint can't load. |
...
| 10 | ⏭ next | Workflows — state machines, triggers, actions (introduces ARQ as first-class infra) |
